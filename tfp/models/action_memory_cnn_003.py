...
MODEL_SPEC = {
    "key": "action_memory_cnn_003",
    "display_name": "Action-Memory CNN 003",
    "algorithm": "ppo",
    "recurrent": False,
    ...
    "description": (
        ...
        "5x5 observation; the model internally embeds recent executed actions, temporally encodes "
        "them, and gates the visual feature before policy/value prediction."
    ),
}
...
class ActionMemoryCNN003(nn.Module):
...
def create_model(observation_shape: tuple[int, int, int], action_count: int) -> nn.Module:
    return ActionMemoryCNN003(observation_shape, action_count)

...
from tfp.checkpoints import checkpoint_path
...
class TFPStudio(tk.Tk):
    ...
    def _build_vars(self) -> None:
        ...
        self.model_var = tk.StringVar(value="simple_cnn_001")
        self.policy_var = tk.StringVar(value="ppo_categorical_001")
        self.reward_var = tk.StringVar(value="dense_transport_001")
        ...
        self.eval_mask_var = tk.StringVar(value="task")
        self.eval_policy_var = tk.StringVar(value="ppo_categorical_001")
        self.eval_reward_var = tk.StringVar(value="dense_transport_001")
    ...
    def _build_ui(self) -> None:
        ...
        SoftRoundedButton(title_row, "Save Config", self._save_config, width=112, height=34, surface=PALETTE["bg"]).pack(side="right", padx=(8, 0))
        SoftRoundedButton(
    ...
    def _build_train_tab(self, parent: ttk.Frame) -> None:
        ...
        self._row_combo(config, r, "Observation", self.obs_var, ["local", "global"]); r += 1
        self._row_entry(config, r, "Local view", self.view_var); r += 1
        self._row_combo(config, r, "Action mask", self.mask_var, ["task", "valid", "none"]); r += 1
    ...
    def _build_eval_tab(self, parent: ttk.Frame) -> None:
        ...
        self._row_readonly(card, r, "Auto checkpoint", self.eval_checkpoint_var, width=72); r += 1
        ...
        self._row_combo(card, r, "Action mask", self.eval_mask_var, ["task", "valid", "none"]); r += 1
    ...
    def _build_compare_tab(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(
            parent,
            text="Each row is one complete fixed map × seed evaluation. Compare models, policies and rewards using robotics/RL metrics rather than a single score. Inference latency is hardware-specific.",
        ...
        columns = ("tag", "model", "policy", "reward", "suite", "episodes", "params", "inference_ms", "success", "pickup", "steps", "return", "efficiency", "collisions", "invalid", "cycles", "interaction_cycles", "revisit")
        self.compare_tree = ttk.Treeview(table_card, columns=columns, show="headings", selectmode="browse")
        headings = {
            "tag":"Tag", "model":"Model", "policy":"Policy", "reward":"Reward", "suite":"Suite", "episodes":"Episodes",
        ...
        widths = {"tag":125,"model":175,"policy":155,"reward":165,"suite":90,"episodes":72,"params":86,"inference_ms":78,"success":76,"pickup":76,"steps":90,"return":90,"efficiency":80,"collisions":80,"invalid":72,"cycles":72,"interaction_cycles":95,"revisit":88}
        for key in columns:
            self.compare_tree.heading(key, text=headings[key], command=lambda k=key: self._sort_comparison(k, False))
            self.compare_tree.column(key, width=widths[key], minwidth=65, anchor="center" if key not in {"tag","model","policy","reward"} else "w")
    ...
    def _comparison_rows(self):
        rows = []
        candidates = []
        reference_dirs = sorted(p for p in (ROOT / "reference_results").iterdir() if p.is_dir()) if (ROOT / "reference_results").exists() else []
    ...
    def _refresh_comparison(self) -> None:
        ...
        for path, row in self._comparison_rows():
            ...
            values = (row["tag"], row["model"], row["policy"], row["reward"], row["suite"], row["episodes"], params_text, inference_text,
    ...
    def _sync_checkpoint_paths(self) -> None:
        try:
            path = checkpoint_path(self.task_var.get(), self.model_var.get())
            resolved = str(path)
        except Exception:
            resolved = "Auto"
        self.checkpoint_var.set(resolved)
        self.eval_checkpoint_var.set(resolved)
    ...
    def _refresh_plugins(self) -> None:
        ...
        if hasattr(self, "task_combo"):
            ...
            self.eval_policy_combo.configure(values=list(policies.keys()))
            self.eval_reward_combo.configure(values=list(rewards.keys()))
    ...
    def _config_payload(self) -> dict[str, object]:
        return {
            ...
            "observation": self.obs_var.get(), "view_size": self.view_var.get(), "action_mask": self.mask_var.get(), "curriculum": self.curriculum_var.get(),
            ...
            "experiment_tag": self.experiment_tag_var.get(),
        }
    ...
    def _load_config(self) -> None:
        ...
        mapping = {
            ...
            "timesteps": self.timesteps_var, "num_envs": self.num_envs_var, "seed": self.train_seed_var, "observation": self.obs_var,
    ...
    def _build_ppo_config(self) -> PPOConfig:
        return PPOConfig(
            ...
            observation_mode=self.obs_var.get(), view_size=int(self.view_var.get()), action_mask_mode=self.mask_var.get(),
    ...
    def _start_training(self) -> None:
        ...
        checkpoint = checkpoint_path(config.task_id, self.model_var.get())
        self._sync_checkpoint_paths()
        train_maps = discover_maps(task.train_map_dir)
        test_maps = discover_maps(task.test_map_dir)
        self._write_log(
            f"Starting {task.env_id}: model={self.model_var.get()} policy={config.policy_module} reward={config.reward_module} "
            f"maps={len(train_maps)} device={config.device}"
        )
        ...
        def work() -> None:
            try:
                train_ppo(
                    self.model_var.get(), train_maps, test_maps, checkpoint, config,
    ...
    def _start_evaluation(self) -> None:
        if self.worker and self.worker.is_alive():
            messagebox.showinfo("TFP Studio", "An experiment is already running.")
            return
        checkpoint = checkpoint_path(self.task_var.get(), self.model_var.get())
        self._sync_checkpoint_paths()
        ...
        def work() -> None:
            try:
                ...
                loaded_model = str(ckpt.get("model_module", "unknown"))
                if loaded_model != self.model_var.get():
                    raise ValueError(
                ...
                test_maps = discover_maps(task.test_map_dir)
                reward_module = self.eval_reward_var.get() or str(ckpt.get("reward_module", "dense_transport_001"))
                policy_module = self.eval_policy_var.get() or str(ckpt.get("policy_module", "ppo_categorical_001"))
                env = TransportEnv(
                    test_maps,
                ...
                self.log_queue.put(f"Starting evaluation: {task.env_id} · {len(test_maps)} maps x {len(seeds)} seeds = {len(test_maps) * len(seeds)} episodes")
                summary, _, paths = evaluate_policy(
                    model, env, test_maps, device, seeds=seeds, action_mask_mode=self.eval_mask_var.get(),
                    policy_module=policy_module, model_name=str(ckpt.get("model_module", "unknown")), presentation=presentation,
